...
static void led_r_task(void *p) {
    ...
    for (;;) {
        if (!blinking) {
            ...
            if (xSemaphoreTake(xSemaphoreLedR, 0) == pdTRUE) { blinking = 0; gpio_put(LED_PIN_R, 0); continue; }
            gpio_put(LED_PIN_R, 0);
            vTaskDelay(pdMS_TO_TICKS(100));
            if (xSemaphoreTake(xSemaphoreLedR, 0) == pdTRUE) { blinking = 0; gpio_put(LED_PIN_R, 0); }
        }
    }
}
...
static void led_y_task(void *p) {
    ...
    for (;;) {
        if (!blinking) {
            ...
            vTaskDelay(pdMS_TO_TICKS(100));
            if (xSemaphoreTake(xSemaphoreLedY, 0) == pdTRUE) { blinking = 0; gpio_put(LED_PIN_Y, 0); continue; }
            gpio_put(LED_PIN_Y, 0);
            vTaskDelay(pdMS_TO_TICKS(100));
            if (xSemaphoreTake(xSemaphoreLedY, 0) == pdTRUE) { blinking = 0; gpio_put(LED_PIN_Y, 0); }
        }
    }
}
